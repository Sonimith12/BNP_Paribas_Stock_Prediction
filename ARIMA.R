...
ts_data_latest <- ts(sample_data_latest, frequency = 12, start = c(2012, 1), end = c(2023, 2))

#plot(seq_along(sample_data), sample_data, type = "l", col = "blue",
 #    xlab = "Mois", ylab = "Valeur", main = "Ventes 2012 - 2021",
  #   yaxt = "n")  # yaxt = "n" supprime les graduations de l'axe y
# Fit ARIMA model
auto_model <- auto.arima(ts_data)

# Generate forecasts
forecast_values <- forecast(auto_model, h = 12)
forecast_points <- forecast_values$mean

# Combine data
combined_data <- c(ts_data, forecast_points)
time_combined <- time(ts_data) + length(ts_data)
ts_data_result <- ts(combined_data, frequency = 12, start = c(2012, 1), end = c(2022, 12))

# Print the result
print(ts_data_result)
print(ts_data_result - ts_data_latest)

rmse <- sqrt(mean((ts_data_latest - ts_data_result)^2))
print(paste("RMSE for 2022:", rmse))


plot(ts_data_result, type = "l", col = "blue", xlab = "Time", ylab = "Value", main = "Actual vs Predicted")
lines(ts_data_latest, col = "red", lty = 2)
